...
###
# Takes a 9 col candidate file and maps string entries to int id's
...
def process_line(ent_map, ep_map, line, rel_map, label_map, token_counter):
    ...
    return e1_str, e2_str, ep_str, rel_str, label, tokens


def export_line(e1_str, e2_str, ep_str, rel_str, label_str, tokens, ent_map, ep_map, rel_map, label_map, token_map, out):
...
-s <throw away relations longer than this> -c <use char tokens (default is use words)> -d <double vocab depending on if [A1 rel A2] or [A2 rel A1]>'
    ...
    if load_vocab_file:
        with open(load_vocab_file, 'rb') as fp:
            [ent_map, ep_map, rel_map, label_map, token_map, token_counter] = pickle.load(fp)
        if reset_tokens:
    ...
    out = open(out_file, 'w')
    [export_line(e1_str, e2_str, ep_str, rel_str, label, tokens, ent_map, ep_map, rel_map, label_map, token_map, out)
     for e1_str, e2_str, ep_str, rel_str, label, tokens in data if len(tokens) <= max_seq]
    out.close()
