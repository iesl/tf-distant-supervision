...
#
flags = tf.flags
flags.DEFINE_string("in_file", 'data/data-min25.ints', "input file")
...
with open(FLAGS.in_file) as f:
    for i, line in enumerate(f):
        ...
        if len(token_list) <= seq_len:
            if len(token_list) < seq_len:
                token_list += [FLAGS.pad_token] * (seq_len - len(token_list))
...
with tf.device('/gpu:'+str(FLAGS.gpuid)):
    ...
    # state = tf.slice(state, [0, 0], [cur_size, FLAGS.hidden_dim])
    state = tf.slice(state, [0, 0], [FLAGS.batch_size, FLAGS.hidden_dim])
    ...
    # _cost = tf.reduce_sum(loss) / cur_size
    _cost = tf.reduce_sum(loss) / FLAGS.batch_size
